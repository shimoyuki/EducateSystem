...
function del(id,admcode,year){
	if(confirm('确认要删除?')){			 
        	 $.get("<%=basePath%>policy/delPolicy?id=" + id+"&"+"admcode=" + admcode+"&"+"year="+year,function(data){  
...
function findYear(){ 
    ...
    form.action = "<%=basePath%>policy/getPolicyList";    
    form.method="post";    
    form.submit();   
    }
...
function download(){
    var form = document.getElementById("myForm");    
    form.action="<%=basePath%>policy/download";    
    form.method="post";    
    form.submit();  
}
</script>
...
<!--添加删除修改-->
<div class="id_12">
<div class="id_13"><span>举办者履职</span><i></i><a href="<%=basePath%>policy/getPolicyList">政策措施</a></div>
<c:if test="${level < 3}">
...
<div class="id_14">
	...
	<c:if test="${level eq 1}">
		<a href="<%=basePath%>policy/download?year=${year}" >下载</a>
		<a href="<%=basePath%>policy/toAddPolicy" class="id_16"></a>
	</c:if>
  <div class="id_18">	
      ...
      <tbody>
           <c:if test="${!empty pageInfo }">  
                <c:forEach items="${pageInfo.list}" var="policy">  
                  <c:if test="${(policy.audit==1&&level eq 3)||level eq 2||level eq 1}">
                    <tr>  
                        ...
                        <td>${policy.audit=="0"?"未审核":"已审核"}</td>
                         ...
                         <c:if test="${level eq 2 }">
                        	<c:if test="${policy.audit==0}">
                        		<td>
                        			<a href="<%=basePath%>policy/updateAudit?id=${policy.id}" class="audit">通过</a>
                        		</td>
                        	</c:if>
                        	<c:if test="${policy.audit==1}">
                        		<td>通过</td>                      		                    		
                        	</c:if>                       
                        </c:if>
                        ...
                        <c:if test="${level eq 1 }">
                          <c:if test="${policy.audit==0}">   
                            <a href="<%=basePath%>policy/getPolicy?id=${policy.id}&detailFlag=0" class="id_23">修改</a>  
                            <a href="javascript:del('${policy.id }','${policy.admcode }','${policy.year }')" class="id_23">删除</a>
                          </c:if>
                          <c:if test="${policy.audit==1}">   
                            ...
                            <a href="<%=basePath%>policy/getPolicyDetail?id=${policy.id}&detailFlag=1" class="id_23">更多</a>  
                        </td>  
                    </tr> 
                   </c:if>              
                </c:forEach>  
...
<nav aria-label="Page navigation"> 
	<ul class="pagination">
		<li><a href="<%=basePath%>policy/getPolicyList?pn=1&years=${year}&schoolNames=${schoolName}">首页</a></li> 
		
		<c:if test="${pageInfo.hasPreviousPage}">
			<li> 
			<a href="<%=basePath%>policy/getPolicyList?pn=${pageInfo.pageNum-1}&years=${year}&schoolNames=${schoolName}" aria-label="Previous"> 
		...
		<c:forEach items="${pageInfo.navigatepageNums}" var="pageNum">
			...
			<c:if test="${pageNum!=pageInfo.pageNum}">
			<li><a href="<%=basePath%>policy/getPolicyList?pn=${pageNum}&years=${year}&schoolNames=${schoolName}">${pageNum}</a></li>
			</c:if>
		</c:forEach> 
		
		<c:if test="${pageInfo.hasNextPage}">
			<li> 
				<a href="<%=basePath%>policy/getPolicyList?pn=${pageInfo.pageNum+1}&years=${year}&schoolNames=${schoolName}" aria-label="Next"> 
					<span aria-hidden="true">&raquo;</span> 
				</a> 
			</li>
		</c:if>
		
		<li><a href="<%=basePath%>policy/getPolicyList?pn=${pageInfo.pages}&years=${year}&schoolNames=${schoolName}">末页</a></li> 
	</ul> 
</nav>
</div>
