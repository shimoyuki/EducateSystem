...
function del(id,admcode,year){
	if(confirm('确认要删除?')){			
		 $.get("<%=basePath%>funds/delFunds?id=" + id+"&"+"admcode=" + admcode+"&"+"year="+year,function(data){
...
function findYear(){ 
    ...
    form.action = "<%=basePath%>funds/getFundsList";    
    form.method="post";    
    form.submit();   
}
...
function download(){
    var form = document.getElementById("myForm");    
    form.action="<%=basePath%>funds/download";    
    form.method="post";    
    form.submit();  
}
</script>
...
<div class="id_14">
	...
	<c:if test="${level eq 1}">
		<a href="<%=basePath%>funds/download?year=${year}" >下载</a>
		<a href="<%=basePath%>funds/toAddFunds" class="id_16"></a>
	</c:if>
  <div class="id_18">	
...
<table class="id_22">
      ...
      <tbody>
           <c:if test="${!empty pageInfo }">  
                <c:forEach items="${pageInfo.list}" var="funds"> 
                  <c:if test="${(funds.audit==1&&level eq 3)||level eq 2||level eq 1}"> 
                    <tr>  
                        <td>${funds.admcode}</td>  
                        <td>${funds.year}</td>                       
                        ...
                        <c:if test="${level eq 2 }">
                        	<c:if test="${funds.audit==0}">
                        		<td>
                        			<a href="<%=basePath%>funds/updateAudit?id=${funds.id}" class="audit">通过</a>
                        		</td>
                        	</c:if>
                        	<c:if test="${funds.audit==1}">
                        		<td>通过</td>                      		                    		
                        	</c:if>                       
                        </c:if>
                        ...
                        <c:if test="${level eq 1 }">
                          <c:if test="${funds.audit==0}">   
                            <a href="<%=basePath%>funds/getFunds?id=${funds.id}&detailFlag=0" class="id_23">修改</a>  
                            <a href="javascript:del('${funds.id }','${funds.admcode }','${funds.year }')" class="id_23">删除</a>
                          </c:if>
                          <c:if test="${funds.audit==1}">   
                            ...
                            <a href="<%=basePath%>funds/getFundsDetail?id=${funds.id}&detailFlag=1" class="id_23">更多</a>  
                        </td>  
                    </tr> 
                   </c:if>              
                </c:forEach>  
...
<nav aria-label="Page navigation"> 
	<ul class="pagination">
		<li><a href="<%=basePath%>funds/getFundsList?pn=1&years=${year}&schoolNames=${schoolName}">首页</a></li> 
		
		<c:if test="${pageInfo.hasPreviousPage}">
			<li> 
			<a href="<%=basePath%>funds/getFundsList?pn=${pageInfo.pageNum-1}&years=${year}&schoolNames=${schoolName}" aria-label="Previous"> 
		...
		<c:forEach items="${pageInfo.navigatepageNums}" var="pageNum">
			...
			<c:if test="${pageNum!=pageInfo.pageNum}">
			<li><a href="<%=basePath%>funds/getFundsList?pn=${pageNum}&years=${year}&schoolNames=${schoolName}">${pageNum}</a></li>
			</c:if>
		</c:forEach> 
		
		<c:if test="${pageInfo.hasNextPage}">
			<li> 
				<a href="<%=basePath%>funds/getFundsList?pn=${pageInfo.pageNum+1}&years=${year}&schoolNames=${schoolName}" aria-label="Next"> 
					<span aria-hidden="true">&raquo;</span> 
				</a> 
			</li>
		</c:if>
		
		<li><a href="<%=basePath%>funds/getFundsList?pn=${pageInfo.pages}&years=${year}&schoolNames=${schoolName}">末页</a></li> 
	</ul> 
</nav>
</div>
