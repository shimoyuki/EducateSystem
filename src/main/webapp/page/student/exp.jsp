...
<script type="text/javascript">
	function query() {
		...
		$("#formTitle").attr("action", "<%=basePath%>exp/showExpInfo");
		$("#formTitle").submit();
		return true;
	}
...
<body>
	...
	<div id="id_11">
		...
		<div class="id_12">
			...
			<form id="formTitle" action="<%=basePath%>exp/download" method="get">
				<div class="id_14">
					<c:if test="${level < 3}">
						<div class="id_18">
				...
				<c:if test="${level eq 1}">
					<a href="<%=basePath%>exp/download?year=${year}" >下载</a>
					<a href="<%=basePath%>exp/addExp" class="id_16"></a>
				</c:if>
					...
					<c:if test="${!empty expList }">
						<c:forEach items="${expList}" var="exp">
							<tr class="id_24">
								...
								<td>${exp.lifebest}</td>
								<td>${exp.audit=="0"?"未审核":"已审核"}</td>

								<c:if test="${exp.audit==0}">
								<td>
									<c:if test="${level eq 2 }">
										<a href="#" name="${exp.id}" class="audit">通过</a>
									</c:if>
								</td>
								</c:if>
								...
								<c:if test="${level eq 1 }">
								    <c:if test="${exp.audit==0}">
										<a href="<%=basePath%>exp/updateExp?id=${exp.id}&detailFlag=0" class="id_23">修改</a>
										<a href="javascript:if(confirm('确认要删除吗？'))del('${exp.id }')" class="id_23">删除</a>
									</c:if>
									<c:if test="${exp.audit==1}">
								...
								<a href="<%=basePath%>exp/showExpDetail?id=${exp.id}&detailFlag=1" class="id_23">更多</a>
								</td>
							</tr>
						</c:forEach>
					</c:if>
		...
		<div style="float:right;">
			...
			<ul class="pagination">
				...
				<li><a
					href="<%=basePath%>exp/showExpInfo?pageNum=1&year=${year}&admcode=${admcode}">首页</a></li>

				<c:if test="${pageInfo.hasPreviousPage}">
					<li><a
						href="<%=basePath%>exp/showExpInfo?pageNum=${expPage.prePage}&year=${year}&admcode=${admcode}"
						aria-label="Previous"> <span aria-hidden="true">&laquo;</span>
					</a></li>
				</c:if>


				<c:forEach items="${expPage.navigatepageNums}" var="pageNum">
					<c:if test="${pageNum==expPage.pageNum}">
						<li class="active"><a href="#" onclick="return false;">${pageNum}</a></li>
					</c:if>
					<c:if test="${pageNum!=expPage.pageNum}">
						<li><a
							href="<%=basePath%>exp/showExpInfo?pageNum=${pageNum}&year=${year}&admcode=${admcode}">${pageNum}</a></li>
					</c:if>
				</c:forEach>

				<c:if test="${pageInfo.hasNextPage}">
					<li><a
						href="<%=basePath%>exp/showExpInfo?pageNum=${expPage.nextPage}&year=${year}&admcode=${admcode}"
						aria-label="Next"> <span aria-hidden="true">&raquo;</span>
					</a></li>
				</c:if>

				<li><a
					href="<%=basePath%>exp/showExpInfo?pageNum=${expPage.pages}&year=${year}&admcode=${admcode}">末页</a></li>
			</ul>
			</nav>
		</div>
