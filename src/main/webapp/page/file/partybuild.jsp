...
window.onload=function(){
	...
	function findYear(){
	    ...
	    form.action = "<%=basePath%>file/getFileList?source=partybuild";
		form.method = "post";
		form.submit();
	}
...
<%@ include file="/page/top.jsp"%>
	...
	<div id="id_11">
		...
		<div class="id_12">
			<div class="id_13">
				<span>资料上传</span><i></i><a href="#">党建工作</a>
			</div>		
			<c:if test="${level < 3}">
				...
				<div>	
					...
					<form action="<%=basePath%>file/upload"  method="post" enctype="multipart/form-data">
					   <div style="float:left;height:60px;margin:15px 8px 0">
					   		<input style="display:none" name="source" value="partybuild" />
			...
			<div style="float: right;">
				...
				<ul class="pagination">
					<li><a href="<%=basePath%>file/getFileList?source=partybuild&pn=1">首页</a></li>

					<c:if test="${pageInfo.hasPreviousPage}">
						<li><a href="<%=basePath%>file/getFileList?source=partybuild&pn=${pageInfo.pageNum-1}"
					...
					<c:forEach items="${pageInfo.navigatepageNums}" var="pageNum">
						...
						<c:if test="${pageNum!=pageInfo.pageNum}">
							<li>
								<a href="<%=basePath%>file/getFileList?source=partybuild&pn=${pageNum}">${pageNum}</a>
							</li>
						</c:if>
					</c:forEach>
					...
					<c:if test="${pageInfo.hasNextPage}">
						<li>
							<a href="<%=basePath%>file/getFileList?source=partybuild&pn=${pageInfo.pageNum+1}" aria-label="Next">
							 <span aria-hidden="true">&raquo;</span>
							</a>
						</li>
					</c:if>

					<li><a href="<%=basePath%>file/getFileList?source=partybuild&pn=${pageInfo.pages}">末页</a></li>
				</ul>
				</nav>
			</div>
