	...
	<div id="id_11">
		...
		<div class="id_12">
			<form id="myForm" method="post"
				action="<%=basePath%>chartAnalysis/getEnterpriseChart">
				<div style="overflow: auto;">
					内容： <select id="model" name="content" class="id_40" onchange="selectChange()">
						<option value="schoolenterprise">合作情况</option>
						<option value="internship">学生实习情况</option>
						<option value="groupschool">集团化办学情况</option>
					</select> 
					年份： <input type="text" id="year" name="year" class="id_40"
						value="${yearlist }"
						onfocus="document.getElementById('light0').style.display='block';"
						onclick="getYear()" /> 地区： <input type="text" id="area"
						name="area" class="id_40" value="${arealist }"
						onfocus="document.getElementById('light3').style.display='block';"
						onclick="getArea()" /> 指标： <input type="text" id="target"
								...
								for ( var per in dataList[adm]) {
									if (dataList[adm][per]['year'] == year[i]) {
										...
										if (typeof(value) ==  'string' && value.indexOf("%") >= 0) {
											...
											value = parseFloat(value.substring(0, value.length));
										}
