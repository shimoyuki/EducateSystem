	...
	<div id="id_11">
		...
		<div class="id_12">
			<form id="myForm" method="post"
				action="<%=basePath%>chartAnalysis/getBasicChart">
				<div style="overflow: auto;">
					内容： <select id="model" name="content" class="id_40" onchange="selectChange()">
						<option value="size">规模</option>
						<option value="equitment">设施设备</option>
						<option value="teachers">教师队伍</option>
						<option value="information">信息化建设</option>
					</select> 
		...
		<div style="overflow: auto;position:absolute;z-index:1;text-align:center;" id="flow1">
			<div id="light0" style="display: none; margin-top:50px; margin-left: 300px; background-color: white; border: 16px solid #0978ad;">
							...
							for ( var adm in dataList) {
								for ( var per in dataList[adm]) {
									if (dataList[adm][per]['year'] == year[i]) {
										...
										//eval('pieData.push([dataList[adm][per]["admcode"], dataList[adm][per]["'+ targetField[j] + '"]])');
										pieSerie['type'] = 'pie';
							...
							for (var j = 0; j < targetField.length; j++) {
								...
								for ( var per in dataList[adm]) {
									if (dataList[adm][per]['year'] == year[i]) {
										...
										data.push(value);
										//eval('data.push(dataList[adm][per]["'+ targetField[j] + '"])');
										match = true;
									}
								}
								if (!match) {
