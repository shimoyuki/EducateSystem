...
function del(id){
	if(confirm('确认要删除?')){
    $.get("<%=basePath%>majorNum/delMajorNum?id=" + id,function(data){  
...
function findYear(){
    ...
    form.action = "<%=basePath%>majorNum/getMajorNumList";    
    form.method="post";    
    form.submit();   
    }
...
function download(){
    var form = document.getElementById("myForm");    
    form.action="<%=basePath%>majorNum/download";    
    form.method="post";    
    form.submit();  
}
</script>
...
<div class="id_14">
	...
	<c:if test="${level eq 1}">
		<a href="<%=basePath%>majorNum/download?year=${year}" >下载</a>
		<a href="<%=basePath%>majorNum/toAddMajorNum" class="id_16"></a>
	</c:if>
  <div class="id_18">	
...
<table class="id_22">
      ...
      <tbody>
           <c:if test="${!empty pageInfo }">  
                <c:forEach items="${pageInfo.list}" var="majorNum">  
                  <c:if test="${(majorNum.audit==1&&level eq 3)||level eq 2||level eq 1}"> 
                    <tr>  
                        ...
                        <c:if test="${level eq 2 }">
                        	<c:if test="${majorNum.audit==0}">
                        		<td>
                        			<a href="<%=basePath%>majorNum/updateAudit?id=${majorNum.id}" class="audit">通过</a>
                        		</td>
                        	</c:if>
                        	<c:if test="${majorNum.audit==1}">
                        		<td>通过</td>                      		                    		
                        	</c:if>                       
                        </c:if>
                        ...
                        <c:if test="${level eq 1 }">
                          <c:if test="${majorNum.audit==0}">   
                            <a href="<%=basePath%>majorNum/getMajorNum?id=${majorNum.id}&detailFlag=0" class="id_23">修改</a>  
                            <a href="javascript:del('${majorNum.id }')" onclick="return confirm('确定删除?');" class="id_23">删除</a>
                          </c:if>
                          <c:if test="${majorNum.audit==1}">   
                            ...
                            <a href="<%=basePath%>majorNum/getMajorNum?id=${majorNum.id}&detailFlag=1" class="id_23">更多</a>  
                        </td>  
                    </tr> 
                  </c:if>              
                </c:forEach>  
...
<nav aria-label="Page navigation"> 
	<ul class="pagination">
		<li><a href="<%=basePath%>majorNum/getMajorNumList?pn=1&years=${year}&schoolNames=${schoolName}">首页</a></li> 
		
		<c:if test="${pageInfo.hasPreviousPage}">
			<li> 
			<a href="<%=basePath%>majorNum/getMajorNumList?pn=${pageInfo.pageNum-1}&years=${year}&schoolNames=${schoolName}" aria-label="Previous"> 
		...
		<c:forEach items="${pageInfo.navigatepageNums}" var="pageNum">
			...
			<c:if test="${pageNum!=pageInfo.pageNum}">
			<li><a href="<%=basePath%>majorNum/getMajorNumList?pn=${pageNum}&years=${year}&schoolNames=${schoolName}">${pageNum}</a></li>
			</c:if>
		</c:forEach> 
		
		<c:if test="${pageInfo.hasNextPage}">
			<li> 
				<a href="<%=basePath%>majorNum/getMajorNumList?pn=${pageInfo.pageNum+1}&years=${year}&schoolNames=${schoolName}" aria-label="Next"> 
					<span aria-hidden="true">&raquo;</span> 
				</a> 
			</li>
		</c:if>
		
		<li><a href="<%=basePath%>majorNum/getMajorNumList?pn=${pageInfo.pages}&years=${year}&schoolNames=${schoolName}">末页</a></li> 
	</ul> 
</nav>
</div>
