...
function del(id){
	if(confirm('确认要删除?')){
    $.get("<%=basePath%>educationTrain/delEducationTrain?id=" + id,function(data){  
...
function findYear(){
    ...
    form.action = "<%=basePath%>educationTrain/getEducationTrainList";    
    form.method="post";    
    form.submit();   
    }
...
function download(){
    var form = document.getElementById("myForm");    
    form.action="<%=basePath%>educationTrain/download";    
    form.method="post";    
    form.submit();  
}
</script>
...
<div class="id_14">
	...
	<c:if test="${level eq 1}">
		<a href="<%=basePath%>educationTrain/download?year=${year}" >下载</a>
		<a href="<%=basePath%>educationTrain/toAddEducationTrain" class="id_16"></a>
	</c:if>
  <div class="id_18">	
...
<table class="id_22">
      ...
      <tbody>
           <c:if test="${!empty pageInfo }">  
                <c:forEach items="${pageInfo.list}" var="educationTrain">  
                  <c:if test="${(educationTrain.audit==1&&level eq 3)||level eq 2||level eq 1}">  
                    <tr>  
                        ...
                        <c:if test="${level eq 2 }">
                        	<c:if test="${educationTrain.audit==0}">
                        		<td>
                        			<a href="<%=basePath%>educationTrain/updateAudit?id=${educationTrain.id}" class="audit">通过</a>
                        		</td>
                        	</c:if>
                        	<c:if test="${educationTrain.audit==1}">
                        		<td>通过</td>                      		                    		
                        	</c:if>                       
                        </c:if>
                        ...
                        <c:if test="${level eq 1 }">
                          <c:if test="${educationTrain.audit==0}">   
                            <a href="<%=basePath%>educationTrain/getEducationTrain?id=${educationTrain.id}&detailFlag=0" class="id_23">修改</a>  
                            <a href="javascript:del('${educationTrain.id }')" onclick="return confirm('确定删除?');" class="id_23">删除</a>
                          </c:if>
                          <c:if test="${educationTrain.audit==1}">   
                            ...
                            <a href="<%=basePath%>educationTrain/getEducationTrain?id=${educationTrain.id}&detailFlag=1" class="id_23">更多</a>  
                        </td>  
                    </tr>
				 </c:if>              
                </c:forEach>  
...
<nav aria-label="Page navigation"> 
	<ul class="pagination">
		<li><a href="<%=basePath%>educationTrain/getEducationTrainList?pn=1&years=${year}&schoolNames=${schoolName}">首页</a></li> 
		
		<c:if test="${pageInfo.hasPreviousPage}">
			<li> 
			<a href="<%=basePath%>educationTrain/getEducationTrainList?pn=${pageInfo.pageNum-1}&years=${year}&schoolNames=${schoolName}" aria-label="Previous"> 
		...
		<c:forEach items="${pageInfo.navigatepageNums}" var="pageNum">
			...
			<c:if test="${pageNum!=pageInfo.pageNum}">
			<li><a href="<%=basePath%>educationTrain/getEducationTrainList?pn=${pageNum}&years=${year}&schoolNames=${schoolName}">${pageNum}</a></li>
			</c:if>
		</c:forEach> 
		
		<c:if test="${pageInfo.hasNextPage}">
			<li> 
				<a href="<%=basePath%>educationTrain/getEducationTrainList?pn=${pageInfo.pageNum+1}&years=${year}&schoolNames=${schoolName}" aria-label="Next"> 
					<span aria-hidden="true">&raquo;</span> 
				</a> 
			</li>
		</c:if>
		
		<li><a href="<%=basePath%>educationTrain/getEducationTrainList?pn=${pageInfo.pages}&years=${year}&schoolNames=${schoolName}">末页</a></li> 
	</ul> 
</nav>
</div>
