...
<script type="text/javascript">
	function query() {
		...
		$("#formTitle").attr("action", "<%=basePath%>teac/showTeacInfo");
		$("#formTitle").submit();
		return true;
	}
...
<body>
	...
	<div id="id_11">
		...
		<div class="id_12">
			...
			<form id="formTitle" action="<%=basePath%>teac/download" method="get">
				<div class="id_14">
					<c:if test="${level < 3}">
						<div class="id_18">
				...
				<c:if test="${level eq 1}">
					<a href="<%=basePath%>teac/download?year=${year}" >下载</a>
					<a href="<%=basePath%>teac/addTeac" class="id_16"></a>
				</c:if>
		...
		<div class="id_21">
			<table class="id_22">
				...
				<tbody>
					<c:if test="${!empty teacList }">
						<c:forEach items="${teacList}" var="teac">
							<tr class="id_24">
								<%-- <td><input type="checkbox" id="checkbox${teac.id}"
										value="${teac.id}" class="chk_2" <c:if test="${teac.audit==1}">disabled="disabled"</c:if>><label
										for="checkbox${teac.id}"></label></td> --%>
								<td>${teac.admcode}</td>
								<td>${teac.year}</td>
								...
								<td>${teac.audit=="0"?"未审核":"已审核"}</td>
								
								<c:if test="${teac.audit==0}">
								<td>
									<c:if test="${level eq 2 }">
										<a href="#" name="${teac.id}" class="audit">通过</a>
									</c:if>
								</td>
								</c:if>
								<c:if test="${teac.audit==1}">
								<td>
									<c:if test="${level eq 2 }">通过</c:if>
								</td>
								</c:if>
								<td>
								<c:if test="${level eq 1 }">
								    <c:if test="${teac.audit==0}">
										<a href="<%=basePath%>teac/updateTeac?id=${teac.id}&detailFlag=0" class="id_23">修改</a>
										<a href="javascript:if(confirm('确认删除？'))del('${teac.id }')" class="id_23">删除</a>
									</c:if>
									<c:if test="${teac.audit==1}">
								...
								<a href="<%=basePath%>teac/showTeacDetail?id=${teac.id}&detailFlag=1" class="id_23">更多</a></td>
							</tr>
						</c:forEach>
					</c:if>
				</tbody>
		...
		<div style="float:right;">
			...
			<ul class="pagination">
				...
				<li><a
					href="<%=basePath%>teac/showTeacInfo?pageNum=1&year=${year}&admcode=${admcode}">首页</a></li>

				<c:if test="${pageInfo.hasPreviousPage}">
					<li><a
						href="<%=basePath%>teac/showTeacInfo?pageNum=${teacPage.prePage}&year=${year}&admcode=${admcode}"
						aria-label="Previous"> <span aria-hidden="true">&laquo;</span>
					</a></li>
				</c:if>


				<c:forEach items="${teacPage.navigatepageNums}" var="pageNum">
					<c:if test="${pageNum==teacPage.pageNum}">
						<li class="active"><a href="#" onclick="return false;">${pageNum}</a></li>
					</c:if>
					<c:if test="${pageNum!=teacPage.pageNum}">
						<li><a
							href="<%=basePath%>teac/showTeacInfo?pageNum=${pageNum}&year=${year}&admcode=${admcode}">${pageNum}</a></li>
					</c:if>
				</c:forEach>

				<c:if test="${pageInfo.hasNextPage}">
					<li><a
						href="<%=basePath%>teac/showTeacInfo?pageNum=${teacPage.nextPage}&year=${year}&admcode=${admcode}"
						aria-label="Next"> <span aria-hidden="true">&raquo;</span>
					</a></li>
				</c:if>

				<li><a
					href="<%=basePath%>teac/showTeacInfo?pageNum=${teacPage.pages}&year=${year}&admcode=${admcode}">末页</a></li>
			</ul>
			</nav>
		</div>
