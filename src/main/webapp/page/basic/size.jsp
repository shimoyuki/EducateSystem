...
function del(id){  
    $.get("<%=basePath%>size/delSize?id=" + id,function(data){  
...
function findYear(){
    ...
    form.action = "<%=basePath%>size/getSizeList";    
    form.method="post";    
    form.submit();  
}
...
function download(){
    var form = document.getElementById("myForm");    
    form.action="<%=basePath%>size/download";    
    form.method="post";    
    form.submit();  
}
</script>
...
<div class="id_14">
	...
	<c:if test="${level eq 1}">
	    <a href="<%=basePath%>size/download?year=${year}" >下载</a>		
		<a href="<%=basePath%>size/toAddSize" class="id_16"></a>
	</c:if>
  <div class="id_18">
...
<table class="id_22">
      ...
      <tbody>
           <c:if test="${!empty pageInfo }">  
                <c:forEach items="${pageInfo.list}" var="size">                   
                   <c:if test="${(size.audit==1&&level eq 3)||level eq 2||level eq 1}">  
                    <tr>  
                        <td>${size.admcode}</td>  
                        <td>${size.year}</td>
                        <td>${size.area}</td>
                        ...
                        <c:if test="${level eq 2 }">
                        	<c:if test="${size.audit==0}">
                        		<td>
                        			<a href="<%=basePath%>size/updateAudit?id=${size.id}" class="audit">通过</a>
                        		</td>
                        	</c:if>
                        	<c:if test="${size.audit==1}">
                        		<td>通过</td>                      		                    		
                        	</c:if>                       
                        </c:if>
                        ...
                        <td> 
                          <c:if test="${level eq 1}">                         
                          	 <c:if test="${size.audit==0}">   
                          		<a href="<%=basePath%>size/getSize?id=${size.id}&detailFlag=0" class="id_23">修改</a>  
                          		<a href="javascript:del('${size.id }')" onclick="return confirm('确定删除?');" class="id_23">删除</a>
                         	 </c:if>
                         	 <c:if test="${size.audit==1}">   
                            ...
                            <a href="<%=basePath%>size/getSize?id=${size.id}&detailFlag=1" class="id_23">更多</a>  
                        </td>  
                    </tr> 
                   </c:if>                             
                </c:forEach>  
...
<nav aria-label="Page navigation"> 
	<ul class="pagination">
		<li><a href="<%=basePath%>size/getSizeList?pn=1&years=${year}&schoolNames=${schoolName}">首页</a></li> 
		
		<c:if test="${pageInfo.hasPreviousPage}">
			<li> 
			<a href="<%=basePath%>size/getSizeList?pn=${pageInfo.pageNum-1}&years=${year}&schoolNames=${schoolName}" aria-label="Previous"> 
		...
		<c:forEach items="${pageInfo.navigatepageNums}" var="pageNum">
			...
			<c:if test="${pageNum!=pageInfo.pageNum}">
			<li><a href="<%=basePath%>size/getSizeList?pn=${pageNum}&years=${year}&schoolNames=${schoolName}">${pageNum}</a></li>
			</c:if>
		</c:forEach> 
		
		<c:if test="${pageInfo.hasNextPage}">
			<li> 
				<a href="<%=basePath%>size/getSizeList?pn=${pageInfo.pageNum+1}&years=${year}&schoolNames=${schoolName}" aria-label="Next"> 
					<span aria-hidden="true">&raquo;</span> 
				</a> 
			</li>
		</c:if>
		
		<li><a href="<%=basePath%>size/getSizeList?pn=${pageInfo.pages}&years=${year}&schoolNames=${schoolName}">末页</a></li> 
	</ul> 
</nav>
</div>
