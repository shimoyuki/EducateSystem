...
window.onload=function(){
	...
	$.ajax({
        url: "<%=basePath%>schoolenterprise/getSchoolNameList",            
...
function del(id){  
	if(confirm('确认要删除?')){
    $.get("<%=basePath%>schoolenterprise/delSchoolenterprise?id=" + id,function(data){  
...
function findYear(){ 
    ...
    form.action = "<%=basePath%>schoolenterprise/getSchoolenterpriseList";    
    form.method="post";    
    form.submit();   
}
function download(){
    ...
    form.action="<%=basePath%>schoolenterprise/download"; 
    form.method="post";    
    form.submit();  
}
</script>
...
<div class="id_14">
	...
	<c:if test="${level eq 1}">
		<a href="<%=basePath%>schoolenterprise/download?year=${year}" >下载</a>		
		<a href="<%=basePath%>schoolenterprise/toAddSchoolenterprise" class="id_16"></a>
	</c:if>
  <div class="id_18">	
...
<table class="id_22">
      ...
      <tbody>
           <c:if test="${!empty pageInfo }">  
                <c:forEach items="${ pageInfo.list }" var="schoolenterprise">  
                    <tr>  
                        ...
                        <td>${schoolenterprise.audit=="0"?"未审核":"已审核"}</td>
                        
						 <c:if test="${level eq 2 }">
                        	<c:if test="${schoolenterprise.audit==0}">
                        		<td>
                        			<a href="<%=basePath%>schoolenterprise/updateAudit?id=${schoolenterprise.id}" class="audit">通过</a>
                        		</td>
                        	</c:if>
                        ...
                        <td> 
                          <c:if test="${level eq 1}">                         
                          	 <c:if test="${schoolenterprise.audit==0}">   
                          		<a href="<%=basePath%>schoolenterprise/getSchoolenterprise?id=${schoolenterprise.id}&detailFlag=0" class="id_23">修改</a>  
                          		<a href="javascript:del('${schoolenterprise.id }')" class="id_23">删除</a>
                         	 </c:if>
                         	 <c:if test="${schoolenterprise.audit==1}">   
                            ...
                            <a href="<%=basePath%>schoolenterprise/getSchoolenterprise?id=${schoolenterprise.id}&detailFlag=1" class="id_23">更多</a>  
                        </td>  
                    </tr>               
                </c:forEach>  
            </c:if>  
...
<nav aria-label="Page navigation"> 
	<ul class="pagination">
		<li><a href="<%=basePath%>schoolenterprise/getSchoolenterpriseList?pn=1">首页</a></li> 
		
		<c:if test="${pageInfo.hasPreviousPage}">
			<li> 
			<a href="<%=basePath%>schoolenterprise/getSchoolenterpriseList?pn=${pageInfo.pageNum-1}" aria-label="Previous"> 
		...
		<c:forEach items="${pageInfo.navigatepageNums}" var="pageNum">
			...
			<c:if test="${pageNum!=pageInfo.pageNum}">
			<li><a href="<%=basePath%>schoolenterprise/getSchoolenterpriseList?pn=${pageNum}">${pageNum}</a></li>
			</c:if>
		</c:forEach> 
		
		<c:if test="${pageInfo.hasNextPage}">
			<li> 
				<a href="<%=basePath%>schoolenterprise/getSchoolenterpriseList?pn=${pageInfo.pageNum+1}" aria-label="Next"> 
					<span aria-hidden="true">&raquo;</span> 
				</a> 
			</li>
		</c:if>
		
		<li><a href="<%=basePath%>schoolenterprise/getSchoolenterpriseList?pn=${pageInfo.pages}">末页</a></li> 
	</ul> 
</nav>
</div>
