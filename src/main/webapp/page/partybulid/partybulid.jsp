...
window.onload=function(){
	...
	$.ajax({
        url: "<%=basePath%>partybulid/getSchoolNameList",            
...
function del(id){  
    $.get("<%=basePath%>partybulid/delPartyBulid?id=" + id,function(data){  
...
function findYear(){
    ...
    form.action = "<%=basePath%>partybulid/getPartyBulidList";    
    form.method="post";    
    form.submit();  
}
...
function download(){
    var form = document.getElementById("myForm");    
    form.action="<%=basePath%>partybulid/download";    
    form.method="post";    
    form.submit();  
}
</script>
...
<!--添加删除修改-->
<div class="id_12">
<div class="id_13"><span>党建工作</span><i></i><a href="#">学校党建工作</a></div>
<c:if test="${level < 3}">
...
<div class="id_14">
	...
	<c:if test="${level eq 1}">
		 <a href="<%=basePath%>partybulid/download?year=${year}" >下载</a>		
		<a href="<%=basePath%>partybulid/toAddPartybulid" class="id_16"></a>
	</c:if>
   <div class="id_18">
...
<table class="id_22">
      ...
      <tbody>
           <c:if test="${!empty pageInfo }">  
                <c:forEach items="${pageInfo.list}" var="partybulid">  
                    <tr>  
                        ...
                        <td>${partybulid.audit=="0"?"未审核":"已审核"}</td>
                        
                        <c:if test="${level eq 2 }">
                        	<c:if test="${partybulid.audit==0}">
                        		<td>
                        			<a href="<%=basePath%>partybulid/updateAudit?id=${partybulid.id}" class="audit">通过</a>
                        		</td>
                        	</c:if>
                        ...
                        <td> 
                          <c:if test="${level eq 1}">                         
                          	 <c:if test="${partybulid.audit==0}">   
                          		<a href="<%=basePath%>partybulid/getPartyBulid?id=${partybulid.id}&detailFlag=0" class="id_23">修改</a>  
                          		<a href="javascript:del('${partybulid.id }')" onclick="return confirm('确定删除?');" class="id_23">删除</a>
                         	 </c:if>
                         	 <c:if test="${partybulid.audit==1}">   
                            ...
                            <a href="<%=basePath%>partybulid/getPartyBulid?id=${partybulid.id}&detailFlag=1" class="id_23">更多</a>  
                        </td>  
                    </tr>               
                </c:forEach>  
            </c:if>  
...
<nav aria-label="Page navigation"> 
	<ul class="pagination">
		<li><a href="<%=basePath%>partybulid/getPartyBulidList?pn=1&years=${year}&schoolNames=${schoolName}">首页</a></li> 
		
		<c:if test="${pageInfo.hasPreviousPage}">
			<li> 
			<a href="<%=basePath%>partybulid/getPartyBulidList?pn=${pageInfo.pageNum-1}&years=${year}&schoolNames=${schoolName}" aria-label="Previous"> 
		...
		<c:forEach items="${pageInfo.navigatepageNums}" var="pageNum">
			...
			<c:if test="${pageNum!=pageInfo.pageNum}">
			<li><a href="<%=basePath%>partybulid/getPartyBulidList?pn=${pageNum}&years=${year}&schoolNames=${schoolName}">${pageNum}</a></li>
			</c:if>
		</c:forEach> 
		
		<c:if test="${pageInfo.hasNextPage}">
			<li> 
				<a href="<%=basePath%>partybulid/getPartyBulidList?pn=${pageInfo.pageNum+1}&years=${year}&schoolNames=${schoolName}" aria-label="Next"> 
					<span aria-hidden="true">&raquo;</span> 
				</a> 
			</li>
		</c:if>
		
		<li><a href="<%=basePath%>partybulid/getPartyBulidList?pn=${pageInfo.pages}&years=${year}&schoolNames=${schoolName}">末页</a></li> 
	</ul> 
</nav>
</div>
